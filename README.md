...
4. use_json: Если опция включена, сообщение либо будет помещено в json строку в секцию "body", а HTTP заголовки в секцию "headers". Если опция выключена, тогда тело сообщения будет непосредственно в сообщении кафки, а HTTP заголовки будут размещены в служебном заголовке этого сообщения. По умолчанию false
