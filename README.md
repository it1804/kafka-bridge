...
Для получения статистики по доставленным в кафку сообщениям можно включить отдельный http сервис, который генерирует один раз в секунду статистику в формате json. Также написана утилита для отправки статистики на сервер prometheus (kafka-bridge_exporter). Эта утилита может централизованно забирать и отправлять статистику сразу с нескольких экземпляров kafka-bridge через встроенный в kafka-bridge сервер статистики.
...
##### Примечание:
Парсер HTTP в kafka-bridge не различает сообщения по URL пути, он будет принимать и пересылать все принятые POST сообщения, поэтому необходимо его размещать за фронтендом, например nginx. Пример конфига nginx есть в примерах.
...
"kafka_consumer_options":{
            ...
            "base64_encode_body":false,
            "options":["auto.offset.reset=earliest", "enable.auto.commit=true", "session.timeout.ms=6000"]
         }, 
```
...
```
У каждого сервиса индивидуальные настройки продюсера кафки.
